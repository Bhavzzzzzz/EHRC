...
# --- NEW: FAST SAVING FUNCTION ---
def save_patch_fast(patch_np, filepath):
    """Converts RGB to BGR and saves using high-speed OpenCV C-bindings"""
    bgr_patch = cv2.cvtColor(patch_np, cv2.COLOR_RGB2BGR)
    cv2.imwrite(filepath, bgr_patch)
...
def tile_vsi_image(vsi_path, output_dir, patch_size=224, tissue_threshold=0.4):
    ...
    # --- NEW: THREAD POOL SETUP ---
    # This spins up 16 background workers to handle the hard drive writes
        ...
        for y in range(0, h_0 - patch_size + 1, patch_size):
            for x in range(0, w_0 - patch_size + 1, patch_size):
                ...
                if tissue_ratio >= tissue_threshold:
                    # Read the block from the image (Main thread)
                    patch_np = scene.read_block((x, y, patch_size, patch_size), size=(patch_size, patch_size))
                    
                    # --- NEW: HAND OFF TO BACKGROUND WORKER ---
                    patch_filename = f"{slide_name}_{x}_{y}.png"
                    filepath = os.path.join(slide_output_dir, patch_filename)
                    
                    # Instead of waiting for the save to finish, we submit the task to the executor
                    # and immediately move to the next patch coordinate!
                    futures.append(executor.submit(save_patch_fast, patch_np, filepath))
                    extracted_count += 1

    # Wait for any remaining background saves to finish before closing the slide
...
if __name__ == "__main__":
    INPUT_DIR = os.path.expanduser("~/local_workspace/raw_data")        
    OUTPUT_DIR = os.path.expanduser("~/local_workspace/unnormalized_patches")
    PATCH_SIZE = 224                    
